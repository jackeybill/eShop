  ...
  implements ArticleCategoryDao
{
  public List<ArticleCategory> findRoots(Integer count)
  {
    ...
    TypedQuery localTypedQuery = this.IIIllIlI.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT);
    if (count != null)
  ...
  public List<ArticleCategory> findParents(ArticleCategory articleCategory, Integer count)
  {
    ...
    TypedQuery localTypedQuery = this.IIIllIlI.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT).setParameter("ids", articleCategory.getTreePaths());
    if (count != null)
      localTypedQuery.setMaxResults(count.intValue());
    return localTypedQuery.getResultList();
  }
  ...
  public List<ArticleCategory> findChildren(ArticleCategory articleCategory, Integer count)
  {
    ...
    if (articleCategory != null)
    {
      ...
      localTypedQuery = this.IIIllIlI.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT).setParameter("treePath", "%," + articleCategory.getId() + "," + "%");
    }
    else
    {
      str = "select articleCategory from ArticleCategory articleCategory order by articleCategory.order asc";
      localTypedQuery = this.IIIllIlI.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT);
    }
    if (count != null)
      localTypedQuery.setMaxResults(count.intValue());
  ...
  private List<ArticleCategory> IIIllIlI(List<ArticleCategory> paramList, ArticleCategory paramArticleCategory)
  {
    ...
    if (paramList != null)
    {
      ...
      while (localIterator.hasNext())
      {
        ...
        if (localArticleCategory.getParent() == paramArticleCategory)
        {
          ...
          localArrayList.addAll(IIIllIlI(paramList, localArticleCategory));
        }
      }
    }
